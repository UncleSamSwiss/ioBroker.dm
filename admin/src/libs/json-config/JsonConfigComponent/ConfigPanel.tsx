...
//[SW] import ConfigJsonEditor from './ConfigJsonEditor';
...
const components = {
	...
	ip: ConfigIP,
	// [SW] jsonEditor: ConfigJsonEditor,
...
const styles = (theme) => ({
	...
	paper: {
		margin: 10,
		//height: "calc(100vh - 235px) !important", // [SW]
...
class ConfigPanel extends ConfigGeneric {
	...
	render() {
		...
		if (hidden) {
			...
			return (
				<Grid key={this.props.attr} container className={classes.fullWidth} spacing={2}>
			...
			if (schema.collapsable) {
				content = (
					...
					>
						<AccordionSummary
							...
							className={clsx(
								classes.fullWidth,
								schema.color === "primary" && classes.primary,
								schema.color === "secondary" && classes.secondary,
							)}
						>
				...
				content = (
					...
					>
						<Grid
							container
							className={clsx(classes.fullWidth, this.props.isParentTab && classes.padding)}
							spacing={2}
						>
							{this.renderItems(items, disabled)}
						</Grid>
